...
def curated_cluster_names(
    ...
    """Return stable public labels keyed by cluster id.

    The semantic clusters are computed from embeddings + KMeans, but the public
    names should not drift on every rebuild. TF-IDF terms remain available in
    the `keywords` field; they are not used as public labels.
    """
    return {
        int(cluster_id): CURATED_LABELS_BY_CURRENT_CLUSTER_ID.get(
            int(cluster_id),
            f"Semantic Theme {int(cluster_id) + 1}",
        )
...
def improve_cluster_names_with_llm(
    ...
    """Choose public cluster labels.

    Default behavior is intentionally stable and curated. Set
    PAPER_MAP_LABEL_MODE=llm if you explicitly want Blablador to suggest public
    labels. Even then, bad/duplicate labels fall back to the curated mapping.
    """
    curated_names = curated_cluster_names(records, labels, keywords)
    label_mode = os.getenv("PAPER_MAP_LABEL_MODE", "curated").strip().lower()

    if label_mode != "llm":
        print("[clusters] Using stable curated public labels.")
        return curated_names, "curated-stable-labels"
